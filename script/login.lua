...
function md5.new() return { a = CONSTS[65], b = CONSTS[66], c = CONSTS[67], d = CONSTS[68], pos = 0,  buf = '', update = md5_update, finish = md5_finish } end
function md5.tohex(s) return format("%08x%08x%08x%08x", str2bei(sub(s, 1, 4)), str2bei(sub(s, 5, 8)), str2bei(sub(s, 9, 12)), str2bei(sub(s, 13, 16))) end
...
local UsernameEntry = vgui.Create("DTextEntry", Panel)
UsernameEntry:SetPos(10, 10)
UsernameEntry:AllowInput()
UsernameEntry:SetSize(230, 25)
UsernameEntry:SetValue("")
...
local PasswordEntry = vgui.Create("DTextEntry", Panel)
PasswordEntry:SetPos(10, 40)
PasswordEntry:AllowInput()
PasswordEntry:SetSize(230, 25)
PasswordEntry:SetValue("")
PasswordEntry.Paint = function(self, w, h)
    draw.RoundedBox(0,0,0,w,h,Color(33, 33, 33, 200))
    surface.SetDrawColor(25, 25, 25, 255)
    surface.DrawOutlinedRect(0,0,w,h)
    self:DrawTextEntryText(Color(255, 255, 255, 0), Color(20, 20, 150), Color(100, 100, 100))
end

local LoginButton = vgui.Create("DButton", Menu)
LoginButton:SetText("Login")
LoginButton:SetPos(197, 103)
LoginButton:SetFont("Trebuchet18")
LoginButton:SetSize(75, 20)
LoginButton.DoClick = function()
    user, pass = UsernameEntry:GetValue(), PasswordEntry:GetValue()
...
ForumButton:SetPos(117, 103)
ForumButton:SetFont("Trebuchet18")
ForumButton:SetSize(75, 20)
